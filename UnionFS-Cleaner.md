...
UnionFS Cleaner is a script (by [l3uddz](https://github.com/l3uddz/unionfs_cleaner)) that moves all the content in `/mnt/local/Media/` (see [[Paths| Basics: Paths#unionfs_cleaner]]) to Google Drive after the folder reaches a certain size. This was first set during initial install of Cloudbox via the [[settings.yml|First Time Install: Configuring Settings]] file, but can also be changed later (see below). 
