...
**Note**: **ALL** folders/paths mentioned below are **CASE SENSITIVE** (e.g. `Media` not `media`, `Movies` not `movies`, `TV` not `tv`, `/logs` not `/Logs` etc). This is important or else Plex and other apps will not find your media, properly.
