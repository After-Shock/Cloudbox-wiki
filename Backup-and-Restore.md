...
1. Configure the rest of your [[settings.yml|Configuring Settings]]. 
   ...
   Note: You may use your previous settings.yml file (`/opt/settings.yml`) as a point of reference to update your new one. However, if all the variables are the same (i.e. no new or modified variables), you may just copy it over (`cp /opt/settings.yml ~/cloudbox`).
